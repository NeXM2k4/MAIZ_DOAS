...
def Dte_Upt():
	...
	dat_act=Dat_Upt_Int()
	for dmy in range(3): mod_ipr.Cap_Day_Month_Year[dmy]=int(dat_act[dmy])
	mod_ipr.Tim_Zon_Int=timezone(mod_ipr.Tim_Zon_Pos)
	return dte_upt
...
def Dat_Upt_Int():
	dat_sys=datetime.now() #tim_sys=time system.
	dat_upt=[0,0,0] #tim_upt=date update.
	dat_upt[0]=dat_sys.strftime("%d")
	dat_upt[1]=dat_sys.strftime("%m")
	dat_upt[2]=dat_sys.strftime("%Y")
	return dat_upt	
...
def Mea_Fol(mea_dte_str,mea_tim_str):
	...
	if isdir(mod_ipr.Dir_Sav_Mai_Pth)==True:
		...
		sec_pth_nam.append(mod_ipr.Dir_Sav_Spe_Int)
		sec_pth_nam.append(mod_ipr.Dir_Sav_Ofs)
		sec_pth_nam.append(mod_ipr.Dir_Sav_Ref)
		sec_pth_nam.append(mod_ipr.Dir_Sav_Fit_ODn)
		sec_pth_nam.append(mod_ipr.Dir_Sav_Fit_Set)
		sec_pth_nam.append(mod_ipr.Dir_Sav_Fit_SCD)
		sec_pth_nam.append(mod_ipr.Dir_Sav_Fit_Con)
	...
	if mod_ipr.Fit_SCD==True:
		for i_ele_pul in range(0,mod_ipr.Ele_Ang_Num,1): #i_ele_pul=i-counter elevation pulse.
			for i_azi_pul in range(0,mod_ipr.Azi_Ang_Num,1): #i_azi_apul=i-counter azimutal pulse.
				scd_fil=sec_pth_nam[6]+"SCD"+"_"+str(mod_ipr.Ele_Ang_Val[i_ele_pul])+"_"+str(mod_ipr.Azi_Ang_Val[i_azi_pul])+"."+mod_ipr.Dat_SCD_Typ
				scd_fil_hu=[[mod_ipr.Dat_SCD_CHe[0][0]],[mod_ipr.Dat_SCD_CHe[0][1]]]
				for i_gas in range(mod_ipr.Fit_Gas_Num): 
					for sss in range(6):
						scd_fil_hu[0].append(mod_ipr.Dat_SCD_CHe[sss+1][0]+mod_ipr.Fit_Gas_Lab[i_gas])
						scd_fil_hu[1].append(mod_ipr.Dat_SCD_CHe[sss+1][1])
				if isfile(scd_fil)==False: 
					mod_raw.Wri_Dat_Str_Txt(scd_fil,scd_fil_hu,1+6*mod_ipr.Fit_Gas_Num,2,'w')
				#Fit_Con,Fit_Gas_Num,Fit_Gas_Lab
				if mod_ipr.Fit_Con==True:
					con_fil=sec_pth_nam[7]+"Con"+"_"+str(mod_ipr.Ele_Ang_Val[i_ele_pul])+"_"+str(mod_ipr.Azi_Ang_Val[i_azi_pul])+"."+mod_ipr.Dat_Con_Typ
					con_fil_hu=[[mod_ipr.Dat_Con_CHe[0][0]],[mod_ipr.Dat_Con_CHe[0][1]]]
					for ce in range(2):
						con_fil_hu[0].append(mod_ipr.Dat_Con_CHe[ce+1][0]+mod_ipr.Fit_Gas_Lab[i_gas])
						con_fil_hu[1].append(mod_ipr.Dat_Con_CHe[ce+1][1])	
					if isfile(con_fil)==False: 
						mod_raw.Wri_Dat_Str_Txt(con_fil,con_fil_hu,1+2*mod_ipr.Fit_Gas_Num,2,'w')										
